#!/usr/bin/env bash
#SBATCH --job-name=target_domain_replay_1k
...
case "${SLURM_ARRAY_TASK_ID}" in
    0)
        VARIANT=target_domain_only
        DATA_DIR="$ROOT/results/tflop_target_domain_train_pseudo_labels"
        ;;
    1)
        VARIANT=target_domain75_ptn25
        DATA_DIR="$ROOT/results/tflop_mixed_target_domain2k_ptn667"
        ;;
    2)
        VARIANT=target_domain50_ptn50
        DATA_DIR="$ROOT/results/tflop_mixed_target_domain2k_ptn2k"
        ;;
    *) exit 2 ;;
esac
OUT="$ROOT/results/tflop_target_domain_replay_smoke_1k/$VARIANT"
cd "$ROOT"
