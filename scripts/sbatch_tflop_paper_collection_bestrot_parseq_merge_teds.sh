#!/bin/bash
#SBATCH --job-name=paperbest_parseq_teds
#SBATCH --time=6:00:00
#SBATCH --tasks=1
#SBATCH --nodes=1
...
OUT_DIR=${OUT_DIR:-/cluster/home/trinhwin/vt2/docling/results/tflop_paper_collection_updated_crops_broadbestrot_parseq_public}
...
$PY scripts/report_paper_collection_ocr_style_results.py \
  ...
  --box-legend "Magenta = same PSENet text-region boxes; text source = PARSeq recognizer" \
  --pipeline-label "table PNG -> PSENet text-region detection -> PARSeq text recognition -> TFLOP inference -> official TFLOP TEDS/TEDS-S."

$PY scripts/rescore_paper_collection_gt_canonicalized.py \
  --run-dir "$OUT_DIR" \
  --output-dir "$OUT_DIR/gt_canonicalized_rescore"

echo "Best-rotation PARSeq inference and TEDS complete."
