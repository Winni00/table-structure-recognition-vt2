#!/bin/bash
#SBATCH --job-name=ftn10k_teds_merge
#SBATCH --time=30:00
#SBATCH --tasks=1
#SBATCH --nodes=1
...
out = ftn10k / "comparison_public_1k_10k_summary.json"
readme = ftn10k / "README.md"
...
def fmt(row: dict) -> str:
    ...
    return (
        f"{samples} | {row['teds_s']:.4f} | {row['teds']:.4f} | "
        f"{row['teds_s_1']} | {row['teds_1']}"
    )

lines = [
    "# FTN 10K Fine-tune Full-Val Evaluation",
    "",
    "Same full FinTabNet annotation-input validation bundle as the public and 1K comparisons.",
    "",
...
echo "Merged FTN 10K fine-tuned full annotation TEDS shards and wrote comparison summary."
