#!/usr/bin/env bash
#SBATCH --job-name=target_domain_replay_eval
...
case "${SLURM_ARRAY_TASK_ID}" in
  0) VARIANT=public ;;
  1) VARIANT=target_domain_only ;;
  2) VARIANT=target_domain75_ptn25 ;;
  3) VARIANT=target_domain50_ptn50 ;;
  *) exit 2 ;;
esac
...
if [[ "$VARIANT" == public ]]; then
  ...
  TRAIN_ROOT="$ROOT/results/tflop_target_domain_replay_smoke_1k/$VARIANT/tflop_$VARIANT/from_public_checkpoint_1000steps_lr1e5"
...
run_eval() {
  ...
  local OUT="$ROOT/results/tflop_target_domain_replay_smoke_1k_eval/$VARIANT/$DATASET"
  mkdir -p "$OUT"
...
test -f "$ROOT/results/tflop_target_domain_val_eval_input/READY"
run_eval \
  target_domain_val \
  "$ROOT/results/tflop_target_domain_val_eval_input" \
  "$ROOT/results/tflop_target_domain_val_eval_input/images"
run_eval \
  ptn_val_pse \
...
"$PY" scripts/rescore_paper_collection_gt_canonicalized.py \
  --run-dir "$ROOT/results/tflop_target_domain_replay_smoke_1k_eval/$VARIANT/paper_test" \
  --output-dir "$ROOT/results/tflop_target_domain_replay_smoke_1k_eval/$VARIANT/paper_test/gt_canonicalized_rescore"

touch "$ROOT/results/tflop_target_domain_replay_smoke_1k_eval/$VARIANT/COMPLETE"
