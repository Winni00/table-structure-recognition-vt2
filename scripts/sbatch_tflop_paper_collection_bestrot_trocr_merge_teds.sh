#!/bin/bash
#SBATCH --job-name=paperbest_trocr_teds
#SBATCH --time=6:00:00
#SBATCH --tasks=1
#SBATCH --nodes=1
...
$PY scripts/report_paper_collection_ocr_style_results.py \
  ...
  --box-legend "Magenta = same PSENet text-region boxes; text source = TrOCR recognizer" \
  --pipeline-label "table PNG -> PSENet text-region detection -> TrOCR text recognition -> TFLOP inference -> official TFLOP TEDS/TEDS-S."

$PY scripts/rescore_paper_collection_gt_canonicalized.py \
  --run-dir "$OUT_DIR" \
  --output-dir "$OUT_DIR/gt_canonicalized_rescore"

echo "Best-rotation TrOCR inference and TEDS complete."
